...
def procesUploadFile(update,message,file_name,file_size):
               ...
               txtname = str(file_name).split('.')[0]+'.txt'
               txt = open(txtname,'w')
               ...
               if file_size > maxsize:
                    ...
                    for part in zipfile.files:
                            try:
                                ...
                                data = uploadToCloud(update,message,part,part_size)
                                directurl = data['directurl']
                                txt.write(directurl + '\n')
                            except:pass
               else:
                   data = uploadToCloud(update,message,file_name,file_size)
                   directurl = data['directurl']
                   txt.write(directurl)
               txt.close()
               txt = open(txtname,'r')
               ...
               infotext+= '<b>Nombre : '+file_name+'</b>\n'
               infotext+= '<b>Tamaño : '+sizeof_fmt(file_size)+'</b>\n'
               infotext+= '<b>Estado : 🎞 Tarea Completada!</b>\n'
               infotext+= '<b>Usuario : '+config.CREDENTIALS['username']+'</b>\n'
               infotext+= '<b>Password : '+config.CREDENTIALS['password']+'</b>\n'
               editHtml(message,infotext)
...
def uploadToCloud(update,message,file_name,file_size):
                    ...
                    loged = client.login()
                    if loged:
                        data = client.upload_file(file_name,False)
